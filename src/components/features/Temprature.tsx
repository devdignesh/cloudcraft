        ...
        <CardFooter className="flex justify-between items-end">
          ...
          <div className="flex flex-col items-end space-y-1">
            <div className="flex flex-col items-end">
              <div className=" relative invert-0 dark:invert  image-icon">
                <Image
                  width={8}
                  height={8}
                  className="h-6 w-6 flx items-end sm:h-8 sm:w-8"
                  src={`/icons/${weather[0].icon}.svg`}
                  alt={weather[0].icon}
                />
              </div>
              <span className=" font-semibold">{weather[0].description[0].toUpperCase() + weather[0].description.slice(1)}</span>
